...
const RoleManagement = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8989/api/visitors')
  ...
  const handleAssignRole = async () => {
    ...
    try {
      await axios.post('http://localhost:8989/api/visitors/assign-role', {
  ...
  const handleRemoveRole = async () => {
    try {
      await axios.post('http://localhost:8989/api/remove-role', { email: selectedUser.email });
  ...
  const handleUpdateRole = async () => {
    const finalRole = computeFinalRole();
    try {
      await axios.post('http://localhost:8989/api/update-role', {
        email: selectedUser.email,
        newRole: finalRole
      });
